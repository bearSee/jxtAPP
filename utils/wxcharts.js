...
function drawPieText(series, opts, config, context, radius, center) {
  ...
  var seriesConvert = series.map(function(item) {
    ...
    var text = item.format ? item.format(+item._proportion_.toFixed(2)) : Math.round(item._proportion_ * 100) + '%';
  ...
  seriesConvert.forEach(function(item) {
    ...
    var orginX1 = Math.cos(item.arc) * radius;
    var orginY1 = Math.sin(item.arc) * radius;
  ...
  textObjectCollection.forEach(function(item) {
    ...
    var textStartX = item.start.x < 0 ? textPosition.x - 5 : textPosition.x - 7;
    var textStartY = item.start.y < 0 ? textPosition.y + 15 : textPosition.y - 5;
    // console.log(item.text, item.start.x, item.start.y)
    ...
    context.beginPath();
    context.setFillStyle('#fff');
    context.fillText(item.text, textStartX, textStartY);
    context.closePath();
    context.stroke();
...
function drawPieDataPoints(series, opts, config, context) {
  ...
  series.forEach(function(eachSeries) {
    const isBold = eachSeries.bold;
    context.beginPath();
    context.setLineWidth(2);
    context.setStrokeStyle('#ffffff');
    context.setFillStyle(eachSeries.color);
    context.moveTo(centerPosition.x, centerPosition.y);
    context.arc(centerPosition.x, centerPosition.y, isBold ? radius * 1.24 : radius * 1.2, eachSeries._start_, eachSeries._start_ + 2 * eachSeries._proportion_ * Math.PI);
    context.closePath();
    context.fill();
...
function drawCharts(type, opts, config, context) {
  var _this = this;
  // console.log('type, opts, config, context', type, opts, config, context)
